...
struct Sim
{
	...
	pthread_t tid[SCENARIO_NUM];

	// C) Create mutex and signal variables for synchronisation
	pthread_mutex_t mutex_state[SCENARIO_NUM]; // restricts access to states of threads
	

	pthread_cond_t cond_finished[SCENARIO_NUM];	// signals whether the thread completed the iteration
	pthread_cond_t cond_start[SCENARIO_NUM];
	// TODO END
};
...
static void Sim_CalcActiveIteration(Sim *sim)
{
    ...
    for (int i = 0; i < SCENARIO_NUM; i++) {
    	pthread_mutex_lock(&sim->mutex_state[i]);

		if (sim->states[i] == WAITING) {
			sim->states[i] = WORKING;
	        pthread_cond_signal(&(sim -> cond_start[i]));
		}
		pthread_mutex_unlock(&sim->mutex_state[i]);
    }
    for (int i=0; i < SCENARIO_NUM; i++) {
    	pthread_mutex_lock(&sim->mutex_state[i]);

    	while (sim->states[i] == WORKING) {
        	pthread_cond_wait(&(sim -> cond_finished[i]),&sim->mutex_state[i]);
    	}
		pthread_mutex_unlock(&sim->mutex_state[i]);
    }
    //TODO END
...
static void Sim_Cleanup(Sim *sim)
{
    ...
    for (int i = 0; i < SCENARIO_NUM; i++) {
        pthread_join((sim -> tid[i]), NULL);
        Scenario_DataDestroy(sim->s[i]);

		pthread_mutex_destroy(&sim->mutex_state[i]);
		pthread_cond_destroy(&sim->cond_start[i]);
		pthread_cond_destroy(&sim->cond_finished[i]);
    }
...
static void Sim_Init(Sim *sim, int argc, char **argv)
{
	...
	for (i = 0; i < SCENARIO_NUM; i++)
	{
		...
		Args *s_args = &(sim->args[i]);
		pthread_mutex_init(&(sim->mutex_state[i]), NULL);
		s_args->mutex_states = &(sim->mutex_state[i]);

		pthread_cond_init(&(sim->cond_finished[i]), NULL);
		s_args->cond_finished = &(sim->cond_finished[i]);
        
        pthread_cond_init(&(sim->cond_start[i]),NULL);
        s_args -> cond_start = &(sim->cond_start[i]);

		// A) Start threads, save reference in sim
		sim->s[i] = Scenario_Create(&(sim->args[i]));
        
		pthread_create(&(sim->tid[i]), NULL, Scenario_Main, sim->s[i]);
		// TODO END
	}
}
