...
void * Scenario_Main(void * scenario_p)
{
	...
	{	
        ...
        pthread_mutex_lock(scenario->args.mutex_states);
		while(*scenario->args.state == WAITING) {
			pthread_cond_wait(scenario->args.cond_start, scenario->args.mutex_states);
		}
		pthread_mutex_unlock(scenario->args.mutex_states);

		// check for preemptive exit
        pthread_mutex_lock(scenario->args.mutex_states);
        if(*scenario->args.state == EXIT || *scenario->args.state == EXIT) {
        	pthread_mutex_unlock(scenario->args.mutex_states);
        	break;
        }
        pthread_mutex_unlock(scenario->args.mutex_states);
		...
		//calculate next step
        pthread_mutex_lock(scenario->args.mutex_states);
        if (*scenario->args.state == WORKING) {
        ...
        *scenario->args.state = WAITING;
        pthread_cond_signal(scenario->args.cond_finished);
        pthread_mutex_unlock(scenario->args.mutex_states);
	}
